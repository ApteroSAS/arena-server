    ...
    if(this.roomId!=val.id)
     {
      const data=new Date(val.createDate );
        ...
        let n: number;
        n = setTimeout( () => { this.state.playersFromOtherRooms.set(element.key,player) }, 500) as unknown as number;
    });
     }
    });
   this.presence.publish('newRoomCreated',data);
    this.presence.subscribe('join',(data:any)=>{
      ...
      if(!this.state.playersFromOtherRooms.get(data.sessionId) &&  this.state.players.get(data.sessionId)==null){
        ...
        player.count=0;
        let n: number;
        n = setTimeout( () => { this.state.playersFromOtherRooms.set(data.sessionId,player) }, 500) as unknown as number;
